...
export const deleteRelationPositionAndPrivilege = async (req, res) => {
    try {
        // Input Data
        const data = req.body;
        ...
        if( status ){ // Operation Successfull
            ...
            if( result.length === 0 ){ // No value
                res.status(500).json({ error: "La Relación Posición-Privilegio de Usuario no existe en la base de datos." });
                return;
            }
            
            // --------- Delete Relation ---------
            const status = await UserPositionsPrivileges.physicalDelete(result[0].id);
            if(status){ // If status = true: Elimination Successfull
                res.json(
                    { 
                        status: status, 
                        message: 'Relación Posición-Privilegio de Usuario eliminada exitosamente.'
                    }
                );
            }else{ // Insertion Not Successfull
                res.status(500).json({ error: "Hubo un problema al eliminar la Relación Posición-Privilegio de Usuario." });
            }      
